...
extern int gpu_p_volatile_util(int gpuid,acct_gather_data_t *data)
{
    nvmlReturn_t rc;
    unsigned int device_count = 0;

    // Initialize NVML and get the number of devices
    _nvml_init();
    gpu_p_get_device_count(&device_count);
    ...
    nvmlDevice_t device;
    if (!_nvml_get_handle(0, &device)) {
        error("Failed to get handle for GPU 0");
        return SLURM_ERROR;
    }
    ...
    if (rc != NVML_SUCCESS) {
        error("NVML: Failed to get utilization rates: %s", nvmlErrorString(rc));
        return SLURM_ERROR;
    }

    // Store utilization rates in the provided data structure
    data->size_read = utilization.gpu;
    data->size_write = utilization.memory;
    // Log the GPU utilization and memory usage
	debug2("gpu_nvml util is %u%%", utilization.gpu);
	debug2("gpu_mem_util is %u%%", utilization.memory);
    log_flag(JAG, "GPU 0 Utilization: %u%%, Memory Usage: %u%%",
