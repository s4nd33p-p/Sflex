...
static void _rpc_get_gpu_util(slurm_msg_t *msg);
...
{
	pid_t pid,throttle_pid;
	int i=0;
	int to_stepd[2] = {-1, -1};
	int to_slurmd[2] = {-1, -1};
	// acct_gather_data_t *gpu_util = xcalloc(2,
	// 				  						sizeof(acct_gather_data_t));
	// job_array_resp_msg_t *resp = NULL;
	// int length = snprintf(NULL, 0, "%u", ((batch_job_launch_msg_t *)req)->job_id);
	// char *jobid_str = (char *)malloc(length+1);										
	...
	// char *jobid_str = (char *)malloc(length+1);	
	// int status;
	// info("atleat it was here");
	// snprintf( jobid_str, length+1, "%u" ,((batch_job_launch_msg_t *)req)->job_id);
	// if (((batch_job_launch_msg_t *)req)->needs_flexing > 0) {
	// 		plugin_handle_t p;
	// 		plugin_err_t e;
	// 		char *path="/usr/lib/slurm/gpu_nvml.so";
	...
	// 		acct_gather_data_t *gpu_util = xcalloc(2,
	// 			  sizeof(acct_gather_data_t));
	// 		double interval = ((((batch_job_launch_msg_t *)req)->needs_flexing)*1000000);
	// 		double active_value = interval*((batch_job_launch_msg_t *)req)->active_time;
	// 		double sleep_time 	= interval - active_value;
	// 		info("interval :%lf, active_time : %lf, sleep_time: %lf", interval, active_value,sleep_time);
	// 		i=0;
	// 		while(1) {
	// 			if (slurm_suspend2(jobid_str, resp) !=0 ) {
	// 				break;
	// 			}
	// 			usleep(sleep_time);
				
	// 			gpu_util = xcalloc(2, sizeof(acct_gather_data_t));
	// 			gpu_g_volatile_util(0, gpu_util);
	// 			info("sm util is %d", gpu_util->size_read);
	// 			resp=NULL;
	// 			if (slurm_resume2(jobid_str, resp) !=0)  {
	// 				break;
	// 			}
	// 			xfree(gpu_util);
	// 			usleep(active_value);
...
static void _rpc_batch_job(slurm_msg_t *msg)
{
	...
	debug2("yuhuu came to slurmd with needs_flexing= %d, active_time=%lf",req->needs_flexing, req->active_time);
	if (!_slurm_authorized_user(msg->auth_uid)) {
	...
	info("atleat it was here");
	snprintf( jobid_str, length+1, "%u" ,((batch_job_launch_msg_t *)req)->job_id);
	// if (((batch_job_launch_msg_t *)req)->needs_flexing > 0 && fork()==0) {
	// 		plugin_handle_t p;
	// 		plugin_err_t e;
	// 		char *path="/usr/lib/slurm/gpu_nvml.so";
...
static void _rpc_get_gpu_util(slurm_msg_t *msg)
{
	...
	slurm_msg_t_copy(&resp_msg, msg);
	gpu_g_volatile_util(0, gpu_util);
	resp.job_id 		= ((gpu_util->size_read)*1000) + gpu_util->size_write;
	// resp.job_id			= gpu_util->size_read;   // gpu_core utilization
	// resp.mem_util		= gpu_util->size_write;   // gpu_memory utilization
	resp.return_code	= SLURM_SUCCESS;
	resp_msg.address 	= msg->address;
	resp_msg.msg_type 	= REQUEST_GPU_UTIL;
	resp_msg.data 		= &resp;
