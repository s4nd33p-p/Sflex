...
/* 	build_scontrol_msg takes nodename and new count as arguments 
*	creates an array of 2 strings which will be passed to _update_node_gres function*/
extern char **build_scontrol_msg(char *nodename, uint64_t new_count) {
...
/* From the string of format gres:mps:40 extracts 40 and returns it */
			     ...
			     bitstr_t *exc_core_bitmap)
{
	int rc;
	bitstr_t **exc_cores;
	node_record_t **global_node_ptr = node_record_table_ptr;
	List gres_list_ptr;
	ListIterator gres_iter;
	gres_state_t *current_gres_record;
	gres_node_state_t *current_mps_ptr;
	int available_mps=0, job_mps_request=0;
	int update_val=0;
	char **argv;
	...
	// 			debug2("tres_per_node is %d",job_mps_request);
	// 			// if (job_mps_request > available_mps ) {
	// 			// 	update_val = current_mps_ptr->gres_cnt_found + (job_mps_request - available_mps);
	// 			// 	argv = build_scontrol_msg(current_node_record->node_hostname, update_val);
	...
	debug2("common_job_test returned %d",rc);
