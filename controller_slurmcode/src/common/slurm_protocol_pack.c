			   ...
			   uint16_t protocol_version)
{
	...
	if (protocol_version >= SLURM_23_02_PROTOCOL_VERSION) {
		...
		packstr(msg->tres_bind, buffer);
		packstr(msg->tres_freq, buffer);
		pack32(msg->needs_flexing, buffer);
		packdouble(msg->active_time, buffer);
	} else if (protocol_version >= SLURM_22_05_PROTOCOL_VERSION) {
		pack32(msg->job_id, buffer);
		pack32(msg->het_job_id, buffer);
	...
	*msg = launch_msg_ptr;
	...
	if (protocol_version >= SLURM_23_02_PROTOCOL_VERSION) {
		...
		safe_unpackstr_xmalloc(&launch_msg_ptr->tres_freq, &uint32_tmp,
				       buffer);
		safe_unpack32(&launch_msg_ptr->needs_flexing, buffer);
		safe_unpackdouble(&launch_msg_ptr->active_time, buffer);
	} else if (protocol_version >= SLURM_22_05_PROTOCOL_VERSION) {
		dynamic_plugin_data_t *select_jobinfo;
		safe_unpack32(&launch_msg_ptr->job_id, buffer);
		safe_unpack32(&launch_msg_ptr->het_job_id, buffer);
