...
static int _foreach_get_tot_from_slurmd_conf(void *x, void *arg)
{
	...
	if (strncmp(gres_slurmd_conf->name,"mps",3)==0)	{
		info("it is mps slurmd conf"); 
		gres_slurmd_conf ->count = slurmd_conf_tot->new_count;
	...
	debug2("total gres count is %"PRIu64"",slurmd_conf_tot->gres_cnt);
	return 0;
}
				 ...
				 slurm_gres_context_t *gres_ctx)
{
	...
	slurmd_conf_tot.is_scontrol = false;
	slurmd_conf_tot.new_count = gres_ns->gres_cnt_config ;
	...
	int t = MAX(gres_ns->gres_cnt_config,slurmd_conf_tot.gres_cnt);
	slurmd_conf_tot.gres_cnt=t;
	gres_ns->gres_cnt_config=t;
	// gres_ns->gres_cnt_found=t;
	...
	if (gres_ns->gres_cnt_found != slurmd_conf_tot.gres_cnt) {
		...
		if ((gres_ns->gres_cnt_found != NO_VAL64) &&
			...
			gres_ns->gres_cnt_found = slurmd_conf_tot.gres_cnt;
			updated_config = true;
			debug2("gres_cnt_found was set to %"PRIu64"",gres_ns->gres_cnt_found);
			debug2("gres_ns->cnt_alloc is %"PRIu64"",gres_ns->gres_cnt_alloc);
		}
	}
			       ...
			       slurm_gres_context_t *gres_ctx)
{
	...
	// }
	debug2("came inside _node_config_test");
	_gres_node_state_delete(new_gres_ns);

	return rc;
}

static int _foreach_update_mps_count(void *x, void *arg) {
    gres_slurmd_conf_t *gres_slurmd_conf = x;
    int new_val = *((int*) arg); // Convert arg back to int*

	if(gres_slurmd_conf == NULL) {
		debug2("oops the list is empty");
	}
    if (strncmp(gres_slurmd_conf->name, "mps", 3) == 0) {
        info("Updating mps GRES count to %d", new_val);
        gres_slurmd_conf->count = new_val;
    }

    return 0;
}
void update_conf(int new_val) {
    tot_from_slurmd_conf_t slurmd_conf_tot;
    ...
    // Iterate over GRES configurations and update the count of "mps" type
    list_for_each(gres_conf_list, _foreach_update_mps_count, &new_val);
}
			  ...
			  bool *updated_gpu_cnt)
{
	...
	*updated_gpu_cnt = false;
	...
	gres_ns->gres_cnt_avail = gres_ns->gres_cnt_config;
	info("gres_ns = %"PRId64,gres_ns->gres_cnt_config);
	info("gres_ns->topo_cnt = %d",gres_ns->topo_cnt);
	info("gres_ns->gres_cnt_avail = %"PRId64,gres_ns->gres_cnt_avail);
	slurmd_conf_tot.is_scontrol = true;
	slurmd_conf_tot.new_count = gres_ns->gres_cnt_config;
	_get_tot_from_slurmd_conf(&slurmd_conf_tot);
	// gres_node_config_validate(gres_state_node->);
	// uint64_t l=gres_node_config_cnt(gres_conf_list, gres_state_node->gres_name);
	// debug2("node_config value is %"PRIu64,l);
	// _merge_gres2(gres_conf_list, gres_state_node->gres_data, gres_ns->gres_cnt_config, gres_state_node->gres_name,gres_ctx,64);
	// update_conf(gres_ns->gres_cnt_config);
			      ...
			      int sock_per_node)
{
	...
	for (i = 0; (rc == SLURM_SUCCESS) && (i < gres_context_cnt); i++) {
		...
		rc = _node_reconfig_test(node_name, new_gres, gres_state_node,
					 &gres_context[i]);
		debug("_node_reconfig_test has returned %d",rc);
	}
	...
	// gres_node_config_validate(node_name, gres_state_node->gres_data ,&new_gres, gres_list, gres_context);
	/* Build new per-node gres_str */
	// need to create Gres list based on gres_str and assign it to gres_list
	// _build_node_gres_str(gres_list, gres_str, cores_per_sock,sock_per_node);
	debug("end gres str %s",*gres_str);

	// now update this new count at slurmd conf file also 
	// list_iterator_create(gres_conf_list);
			      ...
			      bool disable_binding)
{
	...
	while ((gres_state_job = (gres_state_t *) list_next(job_gres_iter))) {
		...
		tmp_cnt = _job_test(gres_state_job, gres_state_node,
				    ...
				    disable_binding);
		debug2("hehe o _job_test in interfaces/gres.c returned %d tp gres_job_test",tmp_cnt);
		if (tmp_cnt != NO_VAL) {
			if (core_cnt == NO_VAL)
				core_cnt = tmp_cnt;
			else
