...
static void _log_node_set(job_record_t *job_ptr,
			  ...
			  int node_set_size);
static void _sflex_test_if_fits(job_record_t *job_ptr,
			  struct node_set *node_set_ptr,
			  int node_set_size);
static uint64_t get_mps_count(char *input);
static void set_mps(char *input, uint64_t new_count);
static char **build_scontrol_msg(char *nodename, uint64_t new_count);
static int slurm_get_util_rpc (pid_t job_pid, uint32_t *sm_util,uint32_t *mem_util, char *nodename);
			...
			bool submission, uint32_t scheduler_type)
{
	...
	// _sflex_test_if_fits(job_ptr, node_set_ptr, node_set_size);
...
static int
slurm_get_util_rpc (pid_t job_pid, uint32_t *jobid, uint32_t *mem_util, char *nodename)
{
	int rc;
	slurm_msg_t req_msg;
	slurm_msg_t resp_msg;
	job_id_request_msg_t req;
	job_id_response_msg_t *recieved_resp;

	slurm_msg_t_init(&req_msg);
	slurm_msg_t_init(&resp_msg);

	slurm_conf_get_addr(nodename, &req_msg.address,
					    req_msg.flags);

	memset(&req, 0, sizeof(req));
	req.job_pid      = job_pid;
	req_msg.msg_type = REQUEST_GPU_UTIL;
	req_msg.data     = &req;
	...
	case REQUEST_GPU_UTIL:
		recieved_resp = ((job_id_response_msg_t *) resp_msg.data);
		*jobid		  = (recieved_resp->job_id)/1000;
		*mem_util 	  = (recieved_resp->job_id)%1000;
		debug2("woohooo reponse of sm_util is %u%% and mem_util is %u%%",*jobid,*mem_util);
		break;
...
extern uint64_t get_mps_count(char *input) {
    char *secondColon = strchr(input, ':');
    if (secondColon != NULL) {
        // Find the position of the next colon after the second colon
        char *thirdColon = strchr(secondColon + 1, ':');
        if (thirdColon != NULL) {
            // Convert the substring after the second colon to a uint64_t
            return strtoull(thirdColon + 1, NULL, 10);
        }
    }
    
    // Return 0 if the second or third colon is not found
    return 0;
}
...
static void set_mps(char *input, uint64_t new_count) {
    ...
    if (secondColon != NULL) {
        ...
        char *thirdColon = strchr(secondColon + 1, ':');
        if (thirdColon != NULL) {
            // Find the end of the current MPS count
            char *end = strchr(thirdColon + 1, ':');
            if (end == NULL) {
                end = input + strlen(input); // If no more colons, go to the end of the string
            }
            
            // Calculate the new length of the input string after replacement
            size_t new_len = (thirdColon + 1 - input) + snprintf(NULL, 0, "%llu", (unsigned long long)new_count) + strlen(end) + 1;
            
            // Create a new string buffer to hold the updated string
            char *new_input = (char *)malloc(new_len);
            if (new_input != NULL) {
                // Copy the part before the third colon
                strncpy(new_input, input, thirdColon + 1 - input);
                new_input[thirdColon + 1 - input] = '\0';
                
                // Append the new MPS count
                snprintf(new_input + (thirdColon + 1 - input), new_len - (thirdColon + 1 - input), "%llu%s", (unsigned long long)new_count, end);
                
                // Copy the new string back to the original input
...
{
	char *node_list, feature_bits[64];
	node_record_t *current_record = NULL;
	List gres_list_ptr;
	ListIterator gres_iter;
	gres_state_t *current_gres_record;
	gres_node_state_t *current_mps_ptr;
	int i,target_mps;
	uint64_t available_mps=0, job_mps_request=0;
	uint64_t update_val=0;
	char **argv;
	uint32_t sm_util = 0, mem_util=10;
	pid_t pid = 1;
	float alpha=1,beta = 5;
	...
	for (i = 0; i < node_set_size; i++) {
		...
		if (job_ptr->tres_per_node != NULL) {
			current_record = find_node_record(node_list);
			if (current_record == NULL ) {
				error("node_record for %s not found",node_list);
			}
			gres_list_ptr = current_record->gres_list;
			...
			while ((current_gres_record = ( gres_state_t *) list_next(gres_iter))) {
				...
				slurm_get_util_rpc(pid, &sm_util, &mem_util, current_record->name);
				if (strncmp(current_gres_record -> gres_name,"mps",3)==0) {
					current_mps_ptr = current_gres_record -> gres_data;
					available_mps = current_mps_ptr->gres_cnt_found - current_mps_ptr->gres_cnt_alloc;
					debug2("Allocated MPS fraction on %s is %"PRIu64"", current_record->node_hostname,
																		current_mps_ptr->gres_cnt_alloc);
					debug2("Availabe MPS fraction on %s is %"PRIu64"", current_record->node_hostname,
																		available_mps );
					job_mps_request = get_mps_count(job_ptr->tres_per_node);
					debug2("extra required mps units %"PRId64"",job_mps_request-available_mps);
					// job_ptr->needs_flexing = 6;
					// job_ptr->active_time = 0.5;
					if (((100-sm_util) > alpha*job_mps_request ) ) {
						debug2("this jobs needs flexing");
						job_ptr->needs_flexing=1;
						job_ptr->active_time = ((double)available_mps)/100;
						// debug2("job_ptr->active_time is %lf",job_ptr->active_time);
						// update_val = current_mps_ptr->gres_cnt_found  + (job_mps_request-available_mps);
						if (available_mps < job_mps_request) {
							update_val = (current_mps_ptr->gres_cnt_config) + ((job_mps_request-available_mps));
							argv = build_scontrol_msg(current_record->node_hostname, update_val);
							_update_node_gres(argv[0], argv[1]);
							// current_mps_ptr->gres_cnt_alloc = 0 ;
							// current_mps_ptr->gres_cnt_avail = update_val;
							free(argv[0]);
							free(argv[1]);
						}
						break;
					...
					// else {
					// 		if (available_mps > job_mps_request) {
					// 		if (current_mps_ptr->gres_cnt_alloc !=0 )
					// 			update_val = current_mps_ptr->gres_cnt_alloc +10;
					// 		else 
					// 			break;
					// 		argv = build_scontrol_msg(current_record->node_hostname, update_val);
					// 		_update_node_gres(argv[0], argv[1]);
					// 		current_mps_ptr->gres_cnt_avail=10;
					// 		free(argv[0]);
					// 		free(argv[1]);
					// 	}
					// }
