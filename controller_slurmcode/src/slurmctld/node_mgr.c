...
extern void	_update_config_ptr(bitstr_t *bitmap,
				   config_record_t *config_ptr);
extern int	_update_node_gres(char *node_names, char *gres);
	...
	while ((config_ptr = list_next(config_iterator))) {
		...
		for (int i = 0; (node_ptr = next_node_bitmap(tmp_bitmap, &i));
			...
			rc2 = gres_node_reconfig(
				...
				node_ptr->cores,
				node_ptr->tot_sockets);
			debug("gres_node_reconfig returned %d",rc2);
			...
			rc2 = gres_node_config_validate(
				node_ptr->name,
				gres,
				&node_ptr->gres, &node_ptr->gres_list,
				node_ptr->threads,
				node_ptr->cores,
				node_ptr->tot_sockets, slurm_conf.conf_flags & CTL_CONF_OR, reason_down);
...
extern void _update_config_ptr(bitstr_t *bitmap, config_record_t *config_ptr)
{
	node_record_t *node_ptr;
