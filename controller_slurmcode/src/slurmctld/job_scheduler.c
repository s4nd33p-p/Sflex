						     ...
						     uint16_t protocol_version)
{
	...
	launch_msg_ptr->needs_flexing = job_ptr->needs_flexing;
	launch_msg_ptr->active_time   = job_ptr->active_time;
...
extern void launch_job(job_record_t *job_ptr)
{
	...
	launch_msg_ptr = _build_launch_job_msg(launch_job_ptr,protocol_version);
		debug2("yuhuuu came to launch_job: ptr->needs_flex=%d, ptr->active_time=%lf",launch_msg_ptr->needs_flexing, launch_msg_ptr->active_time);
	// debug2("tres_per_node is %s",launch_msg_ptr);
